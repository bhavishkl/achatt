...
export async function GET(request: Request) {
  ...
  try {
    let query = supabaseAdmin
      .from('absent_records')
      .select('*, employee_details:employees!absent_records_employee_id_fkey!inner(company_id)')
    ...
    if (error) {
      ...
      return NextResponse.json({ message: 'Error fetching absent records', error: error.message }, { status: 500 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ message: 'Error creating absent record', error: error.message }, { status: 500 });
    }
